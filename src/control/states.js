...
const states = {
  ...
  start: () => {
    ...
    window.console.log('game begin, new id: ', states.id);
    if (music.start) {
      music.start();
    }
  ...
  clearLines: (matrix, lines) => {
    ...
    // window.console.log('news:', state.get('clearLines'), lines.length, clearLines);
    // window.console.log('wallet addr:', sessionStorage.getItem('account'), states.id);
    // const tmpline = {};
    // tmpline.lines = lines.length;
    // tmpline.chain = 0;
    // store.dispatch(actions.tmpLines(tmpline));
    // window.console.log('new line dispatch:', lines.length);
    const addPoints = store.getState().get('points') +
      clearPoints[lines.length - 1]; // 一次消除的行越多, 加分越多
    states.dispatchPoints(addPoints);

    // 如果从普通模式切换过来，id无效，重新生成
    if (!states.id) {
